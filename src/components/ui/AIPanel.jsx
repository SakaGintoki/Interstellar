...
const expertPersona = {
  ...
  universe:
    "You are a cosmology and astrophysics expert specializing in galaxies, spacetime, and the universe.",
};
...
function AIPanel() {
  ...
  useEffect(() => {
    if (isAIPanelOpen) {
      const scaleName =
        currentScale.charAt(0).toUpperCase() + currentScale.slice(1);
      const persona =
        expertPersona[currentScale] ||
        "You are an expert in fundamental science.";

      const welcomeMessage = {
        role: "system",
        content: `${persona} You are now viewing the ${scaleName} scale. Ask me anything in this domain.`,
      };

      if (
        chatHistory.length === 0 ||
        chatHistory[chatHistory.length - 1].content !== welcomeMessage.content
      ) {
  ...
  const handleSubmit = async (message) => {
    ...
    try {
      ...
      console.error(err);
      setError(
        "Failed to get response from AI. Check your server and network."
      );
      const errorMessage = {
        role: "system",
    ...
    >
      <div className="p-6 pt-20 flex-shrink-0">
        ...
        <p className="text-sm text-gray-400">
          Context:{" "}
          <span className="font-bold text-accent-blue-light">
            {currentScale}
          </span>
        </p>
      </div>
